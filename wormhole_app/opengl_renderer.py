...
vec3 trace_tunnel_color(vec3 ray, float camera_l) {
    ...
    float tunnel_phase = clamp((camera_l + a_param) / max(2.0 * a_param, 1e-6), 0.0, 1.0);
    float wall_depth = depth + tunnel_phase * 5.2;
    float u = fract(theta / (2.0 * PI) + 0.5 + wall_depth * 0.028 + 0.018 * sin(wall_depth * 0.55));
    float v = fract(wall_depth * 0.145 + 0.055 * sin(theta + wall_depth * 0.45));
    float u2 = fract(theta / (2.0 * PI) + 0.5 - wall_depth * 0.018 + 0.027 * cos(theta + wall_depth * 0.31));
    float v2 = fract(wall_depth * 0.105 + 0.045 * cos(theta - wall_depth * 0.37));
    float theta_a = theta + (2.0 * PI / 3.0);
    float theta_b = theta - (2.0 * PI / 3.0);
    float u3 = fract(theta_a / (2.0 * PI) + 0.5 + wall_depth * 0.020);
    float v3 = fract(wall_depth * 0.130 + 0.040 * sin(theta_a + wall_depth * 0.41));
    float u4 = fract(theta_b / (2.0 * PI) + 0.5 - wall_depth * 0.014);
    float v4 = fract(wall_depth * 0.118 + 0.036 * cos(theta_b - wall_depth * 0.33));
    vec3 wall = texture(exit_tex, vec2(u, 1.0 - v)).rgb;
    vec3 wall_alt = texture(exit_tex, vec2(u2, 1.0 - v2)).rgb;
    vec3 wall_ring = texture(exit_tex, vec2(u3, 1.0 - v3)).rgb * 0.5 + texture(exit_tex, vec2(u4, 1.0 - v4)).rgb * 0.5;
    ...
    float early_shrink = smoothstep01(0.01, 0.05, tunnel_phase);
    float exit_approach = smoothstep01(0.25, 0.95, tunnel_phase);
    float far_core = mix(0.20, 0.36, exit_approach);
    float core_inner = mix(0.52, far_core, early_shrink);
    float core_outer = core_inner + mix(0.12, 0.08 + 0.04 * exit_approach, early_shrink);
    float rim_radius = core_outer - 0.04;
    float rim_width = mix(0.075, 0.040 + 0.025 * exit_approach, early_shrink);

    float aperture = 1.0 - smoothstep01(core_inner * 0.38, core_inner * 0.95, radial);
    float wall_mix = smoothstep01(0.24, 0.42, radial);
    float longitudinal_glow = 0.5 + 0.5 * cos(wall_depth * PI * 1.4);
    float rib_phase = abs(fract(wall_depth * 0.62) - 0.5) * 2.0;
    ...
    wall = wall * 0.10 + vec3(wall_avg) * 0.62 + vec3(0.14, 0.18, 0.24) * 0.28;
    wall *= (0.72 + 0.28 * ribs) * side_shade * 1.04;
    vec3 core = exit_view;
    float circular_core = 1.0 - smoothstep01(core_inner, core_outer, radial);
    vec3 color = wall * (1.0 - circular_core) + core * circular_core;
    float rim = exp(-pow((radial - rim_radius) / rim_width, 2.0));
